class Portfolio:
    ...
    def to_plotly_format(self):
        ...
        return f"Average Price: {self.price:.3f}<br>Delta: {self.delta:,.4f}<br>Gamma: {self.gamma:,.2f}<br>Vega: {self.vega:,.2f}<br>Theta: {self.theta:,.2f}<br>Total Premium: {self.premium:,.2f}<br>P&L: {self.p_and_l:,.2f}"
